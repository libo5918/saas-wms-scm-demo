...
```
## 15. Phase 3.1 Milvus Adapter 决策

Milvus 接入采用官方 Java SDK：
...
```text
默认 in-memory，不连接 Milvus。
仅当 ai.agent.rag.vector-store.mode=milvus 时创建 MilvusRagVectorStore。
```

这样做的原因：

- 保证本地开发和 CI 不依赖 Milvus。
- 让真实 Milvus 能以 smoke test 方式逐步验证。

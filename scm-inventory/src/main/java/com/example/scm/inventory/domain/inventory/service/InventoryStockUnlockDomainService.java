...
/**
 * 库存解锁领域服务。
 * 负责解锁幂等校验、释放冻结库存以及生成解锁流水。
 */
@Service
public class InventoryStockUnlockDomainService {
    ...
    public InventoryStockUnlockDomainService(InventoryBalanceRepository inventoryBalanceRepository,
     ...
     * 执行单条库存解锁。
     */
    public InventoryTransactionRecord unlock(Long tenantId, String bizType, String bizNo, Long operatorId,
        ...
        if (inventoryTransactionRecordRepository.existsUnlockRecord(tenantId, bizType, bizNo, materialId, warehouseId, locationId)) {
            throw new BusinessException(CommonErrorCode.BAD_REQUEST.code(), "Duplicate stock-unlock request");
        }
        ...
        InventoryTransactionRecord record = balance.unlock(generateTxnNo("UNLOCK", bizType, bizNo, materialId, warehouseId, locationId), bizType, bizNo, quantity, operatorId);
    ...
    private void validateArguments(Long tenantId, String bizType, String bizNo, Long operatorId,
        ...
        if (tenantId == null || operatorId == null || materialId == null || warehouseId == null || locationId == null) {
            throw new BusinessException(CommonErrorCode.BAD_REQUEST.code(), "stock-unlock arguments cannot be null");
        }

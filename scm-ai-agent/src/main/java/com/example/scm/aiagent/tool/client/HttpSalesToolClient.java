...
public class HttpSalesToolClient extends AbstractToolClientSupport implements SalesToolClient {
    ...
    public HttpSalesToolClient(RestClient.Builder restClientBuilder, AiAgentProperties properties) {
        ...
        this.salesBaseUrl = http.getSalesBaseUrl();
    }
        ...
        try {
            ...
            if (orderId != null) {
                result = restClient.get()
                        .uri(salesBaseUrl + "/api/v1/sales-orders/{id}", orderId)
                        .headers(headers -> headers.addAll(identityHeaders(request)))
                        .retrieve()
                        .body(RESULT_TYPE);
            } else if (StringUtils.hasText(orderNo)) {
                result = restClient.get()
                        .uri(salesBaseUrl + "/api/v1/sales-orders/by-order-no?orderNo={orderNo}", orderNo)
                        .headers(headers -> headers.addAll(identityHeaders(request)))
                        .retrieve()
                        .body(RESULT_TYPE);
            } else {
                throw new ToolClientException("Sales tool requires orderId or orderNo");
            }
            if (result == null) {
                throw new ToolClientException("Sales service returned empty response");
            }
            if (!result.success()) {
                throw new ToolClientException("Sales service failed: " + result.message());
            }
            log.info("Sales order tool HTTP call success, tenantId={}, userId={}, runId={}, orderId={}, orderNo={}, latencyMs={}",
            ...
            throw new ToolClientException("Sales service call failed: " + ex.getMessage(), ex);
        }
    }
